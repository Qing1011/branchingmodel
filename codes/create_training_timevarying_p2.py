# create_training_timevarying.py
...
def build_dataset(export_dir, r_list, W_path, t0=10, T=50, g_max=300, round_r=3):
    # 1) load mobility window once
    with h5py.File(W_path, 'r') as f:
        A = f['WM'][t0:t0+T]  # (T,N,N)
    ...
    for r in r_list:
        ...
        # subdir = os.path.join(export_dir, f"branching_pre0.02_post-{r_tag}")
...
if __name__ == "__main__":
    os.makedirs("./training", exist_ok=True)  # ensure dir exists

    r_list = [10.0, 2.0, 1.0, 0.5, 0.2, 0.1, 0.05, 0.025, 5., 2.5, 3.333,
              1.333, 0.667, 0.286, 0.133, 0.067, 0.033, 0.37, 7.4, 20, 13.333]
    ...
    torch.save(A_t, "./training/A_t_2.pt", _use_new_zipfile_serialization=False)

    # Save dataset WITHOUT A_t; use legacy serializer for NFS robustness
    tmp = "./training/.simulation_real_p2.tmp"
    out = "./training/simulation_real_p2.pt"
