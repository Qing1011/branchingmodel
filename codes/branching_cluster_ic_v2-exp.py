...
def main():
    ...
    para_dict = np.load('para_dict-exp.npy', allow_pickle=True)
    # set parameters
    para_i = para_dict[s]
    R0 = para_i['R0']
    r = para_i['r']
    ...
    E_NewInf_i, E_TotInf_i = superspreading_T_Loc_expdelay(
        T, num_fips, (l0, i0), (r, p), pop, (Z, Zb, D, Db), WN, ss)
    E_NewInf[:, :] = E_NewInf_i[:, :T]
    E_TotInf[:, :] = E_TotInf_i[:, :T]
    save_dir = '/rds/general/user/qy1815/ephemeral/python_no_cutoff-exp/branching_R0-{}_r-{}/' .format(
    ...
    f.close()
    # np.savetxt(save_dir+"NewInf_R0-{}_r-{}_{}.csv" .format(
    #     np.round(R0, 2), np.round(r, 3), es_idx), E_NewInf)
