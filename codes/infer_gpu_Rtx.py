...
g_idx = int(sys.argv[4])  # index for realisation idx
...
# data_path = '/insomnia001/depts/msph/users/qy2290/Rtx_nrr_den/gnn_inference_data/r-{}/'.format(my_r)
...
save_stub = f'infer_results/r{my_r}_{g_idx}_seed{seed}_{timestamp}'
...
print(f"r target directory: r-{my_r}_{g_idx}")
print(f"Seed: {seed}")
...
for epoch in range(num_epochs):
    ...
    for batch in train_loader:
        ...
        elbo = model.sample_elbo_loss(
            ...
            sample_nbr=30,
            complexity_cost_weight=kl_weight(epoch, num_epochs, max_w=3e-3, warmup_frac=0.8) ## used to be max_w=1e-3
        )
        elbo.backward()
        # --------------end of changes----------------
        optimizer.step()
