...
def main():
    s = sys.argv[1]
    s = int(s) - 1
    es_idx = int(sys.argv[2])  # this is first 10

    rs = np.array([20, 10., 2, 1, 0.5, 0.2, 0.1, 0.05, 0.025])
    R0s = [2.5]  # finish the rest of R0 values
    ...
    num_fips = len(pop)
    T = 60
    num_ens = 10  # 300 ###500 intially when R0 gets larger, we need fewer ensemble members, std is smaller
    ...
    for i in range(x_cutoff):
        temp1 = SS.gamma(r+i)/SS.gamma(r)/SS.gamma((i+1)) * \
            np.power(p, r)*np.power((1-p), i)
        weights[i] = temp1
    ...
    save_dir = '/rds/general/user/qy1815/ephemeral/branching_results_R025/'

    for en_i in range(num_ens):
        # chose different seed for each ensemble member
        np.random.seed(int(time.time()) + en_i)
        E_NewInf_i, E_TotInf_i = superspreading_T_Loc(
            T, num_fips, (l0, i0), weights_n, pop, (Z, Zb, D, Db), WN)
        ...
        # f.close()
    f = gzip.GzipFile(
        save_dir+"NewInf_R0-{}_r-{}_{}.npy.gz" .format(np.round(R0, 2), np.round(r, 2), es_idx), "w")
    np.save(file=f, arr=E_NewInf)
    f.close()
